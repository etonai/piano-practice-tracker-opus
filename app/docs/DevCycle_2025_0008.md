...
- **Key Difference**: Dialog lifecycle and timing issue
- **Dashboard Flow**: Navigates to full Activity screen → `AddActivityViewModel.saveActivity()` → navigates back to main screen where flows are still active
- **Suggestions Flow**: `QuickAddActivityDialogFragment` → `QuickAddActivityViewModel.addActivity()` → **immediately dismisses dialog** upon repository success
- **Root Cause**: Dialog dismisses before database transaction/flow emissions complete
  - `QuickAddActivityViewModel.addActivity()` posts success immediately after `repository.insertActivity()` call
  - Dialog observer receives success and calls `dismiss()` immediately  
  - Database statistics update (`updatePieceStatistics()`) may still be running in background
  - Flow emissions to UI may not have propagated before dialog dismissal

**Solution Options:**
1. **Delay Dialog Dismissal**: Add small delay before dismissing dialog to allow database/flow completion
2. **Wait for Statistics Update**: Modify `repository.insertActivity()` to ensure `updatePieceStatistics()` completes before returning
3. **Transaction Boundary**: Wrap both activity insertion and statistics update in single transaction
4. **Flow Synchronization**: Wait for piece statistics flow to emit updated data before dismissing
5. **Async/Await Pattern**: Ensure all database operations complete before posting success result
...
- [ ] Implement fix to ensure piece statistics update before dialog dismissal
- [ ] Verify suggestions refresh properly after adding activity from suggestions tab
- [ ] Verify Pieces tab shows updated "last practice" dates immediately  
- [ ] Test that database transaction completes before UI updates
- [ ] Test both scenarios (suggestions tab and normal activity addition) work identically
